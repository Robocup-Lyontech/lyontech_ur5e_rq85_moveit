...
class MoveGroup_Palbator_UR5e(object):
  """MoveGroupTutorial"""

  ORI_FRONT = Quaternion(0, 0.707, 0.707, 0)
  ORI_DOWN = Quaternion(1, 0, 0, 0)

  POINT_STD = Point(0.0, 0.65, 0.4)

  POSE_STD = Pose(POINT_STD, ORI_FRONT)

  def __init__(self):
    super(MoveGroup_Palbator_UR5e, self).__init__()
    ...
    group_name = "ur5e_arm" # See .srdf file to get available group names
  ...
  def go_to_pose_goal(self, p = POSE_STD ):
    # Copy class variables to local variables to make the web tutorials more clear.
    # In practice, you should use the class variables directly unless you have a good
    # reason not to.
    group = self.group
    ...
    #p.orientation.x, p.orientation.y, p.orientation.z, p.orientation.w  = tf.transformations.quaternion_from_euler(pi*0.5, 0, pi)
  ...
  def add_box(self, timeout=4):
    ...
    # box_name = "back"
    # scene.add_box(box_name, box_pose, size=(0.1, 0.5, 1))
    ...
    box_name = "socle"
    scene.add_box(box_name, box_pose, size=(0.52, 0.58, 0.1))
    self.box_name=box_name
    self.wait_for_state_update(box_is_known=True, timeout=timeout)
...
def main():
    try:
        ...
        #input()
        tutorial = MoveGroup_Palbator_UR5e()
